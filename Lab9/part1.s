...
PRIV_TIMER_CHECK:
    			CMP R5, #MPCORE_PRIV_TIMER_IRQ //Check to see if interrupt ID matches table
				BNE KEYS_CHECK //If not, check keys
				BL PRIV_TIMER_ISR //Always proceed to interrupt routine if code passes
				B EXIT_IRQ //Exit interrupt request handler
    
KEYS_CHECK:
    			CMP R5, #KEYS_IRQ //Check to see if interrupt ID matches interrupt table
BAD_NUM:		BNE BAD_NUM //Bad number accepted in KEY, stop code
				BL KEY_ISR //Always proceed to interrupt routine if code passes
...
SWEEP:		LDR		R0, =LEDR_DIRECTION	// put shifting direction into R2
				...
				LDR		R3, [R1]
				MOVW		R12, #0x201	//Reg 12 to store large value for convenience, hacky code, I know, I'm sorry, pls forgive ^-^
				MOVW		R11, #0x102
				CMP R2, #1
				BEQ TOOUTSIDE
				
TOCENTRE:		CMP R3, R12	//I basically use the concept of cases to solve this. There is a less hacky way to do this by adding the rest of the series 0x102, 0x84, 0x48, 0x30 to LEDR_PATTERN, and then cycling through the word array, which would be much cleaner, but I'm doing this the day before it's due, so I'm sorry. Also I don't know if I'm allowed to modify it given that there was no "..." for it, and I'm too shy and embarrased to ask on Piazza because of how late I'm doing this.
				MOVWEQ R3, #0x102	//case 1
				BEQ DONE_SWEEP
				
				CMP R3, R11	//Next case for LEDR_PATTERN, cycle through until last to switch direction at C_O
				MOVEQ R3, #0b0010000100	//case 2
				BEQ DONE_SWEEP
				
				CMP R3, #0b0010000100
				MOVEQ R3, #0b0001001000	//case 3
				BEQ DONE_SWEEP
				
				CMP R3, #0b0001001000
				MOVEQ R3, #0b0000110000	//case 4

C_O:			MOV		R2, #1					// change direction to outside
				BEQ DONE_SWEEP
				
TOOUTSIDE:		CMP R3, #0b0000110000	//Continued concept of cases, just cycle the LEDRs outward now until last case, in which O_C in reached and direction swaps
				MOVEQ R3, #0b0001001000	//case -4
				BEQ DONE_SWEEP
				
				CMP R3, #0b0001001000
				MOVEQ R3, #0b0010000100	//case -3
				BEQ DONE_SWEEP
				
				CMP R3, #0b0010000100
				MOVWEQ R3, #0x102	//case -2
				BEQ DONE_SWEEP
				
				CMP R3, R11
				MOVWEQ R3, #0x201	//case -1
...
START_STOP:		LDR		R0, =MPCORE_PRIV_TIMER		// timer base address
					...
					MVN 	R1, R1					//Invert current status (stop if started, start if stopped)
					STR		R1, [R0, #0x8]			//Store new value
...
*/
			.global	_start
_start:
			MOV R0, #0b11010010 //Enable IRQ and SVC stack pointers
			MSR CPSR, R0
			LDR SP, =0x20000
			MOV R0, #0b11010011
			MSR CPSR, R0
			...
			MOV R0, #0b01010011 //Enable interrupts on SVC
			MSR CPSR, R0
			MOV R0, #0
...
CONFIG_PRIV_TIMER:
			...
			LDR		R1, =20000000			//Timer period
			STR		R1, [R0]				//Store period
			MOV		R1, #0b111				//Enable interrupt and autoreload
			STR		R1, [R0, #8]			//Store previous line
			MOV 		PC, LR 					// return

/* Configure the KEYS to generate an interrupt */
CONFIG_KEYS:
			LDR 		R0, =0xFF200050 		// KEYs base address
			MOV			R1, #0b1111				//Enable all keys
			STR 		R1, [R0, #0x8]			//Store to interrupt mask register
			MOV 		PC, LR 					// return
